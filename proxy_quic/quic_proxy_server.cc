...
QuicProxyServer::QuicProxyServer(
  ...
  if (-1 != idle_network_timeout_) {
    config_.SetIdleNetworkTimeout(QuicTime::Delta::FromSeconds(idle_network_timeout_),
                                  QuicTime::Delta::FromSeconds(idle_network_timeout_/2));
  }
  Initialize();
}

void QuicProxyServer::Initialize() {
  ...
  QuicProxyBackend* proxy_backend =
    reinterpret_cast<QuicProxyBackend*>(quic_simple_server_backend_);
  proxy_backend->set_server(this);
...
QuicProxyServer::~QuicProxyServer() = default;

bool QuicProxyServer::CreateUDPSocketAndListen(const QuicSocketAddress& address) {
  ...
  int boolean_value = 1;
  if (-1 == setsockopt(fd_, SOL_SOCKET, SO_REUSEADDR,
                      reinterpret_cast<const char*>(&boolean_value),
                      sizeof(boolean_value))) {
    LOG(ERROR) << "SetReuseAddr() failed: " << strerror(errno);
    return false;
  }
  ...
  if (port_ == 0) {
    ...
    if (address.FromSocket(fd_) != 0) {
      LOG(ERROR) << "Unable to get self address.  Error: "
                      << strerror(errno);
    }
    port_ = address.port();
  }
...
void QuicProxyServer::WaitForEvents() {
  epoll_server_.WaitForEventsAndExecuteCallbacks();
}

void QuicProxyServer::Shutdown() {
  if (!silent_close_) {
...
void QuicProxyServer::OnEvent(int fd, QuicEpollEvent* event) {
  DCHECK_EQ(fd, fd_);
