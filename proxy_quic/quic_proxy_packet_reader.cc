...
QuicProxyPacketReader::QuicProxyPacketReader() {
#if !MMSG_MORE
...
QuicProxyPacketReader::~QuicProxyPacketReader() = default;

bool QuicProxyPacketReader::ReadAndDispatchPackets(
    int fd,
    int port,
  ...
  for (int i = 0; i < packets_read; ++i) {
    ...
    if (QUIC_PREDICT_FALSE(mmsg_hdr_[i].msg_hdr.msg_flags & MSG_TRUNC)) {
      ...
      QUIC_SERVER_HISTOGRAM_COUNTS(
          "QuicProxyPacketReader.DroppedPacketSize", mmsg_hdr_[i].msg_len, 1, 10000,
          20, "In QuicProxyPacketReader, the size of big packets that are dropped.");
      continue;
    }
